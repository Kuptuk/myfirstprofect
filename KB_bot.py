...
async def on_ready():
  ...
  await client.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="Как жёстко быть слитым девушкой за пару минут: урок от студента Оксфорда за 3к рублей в день"))
...
async def info(message, id = None):
    if id == 'badges':
      ...
      try:
          ...
          if str(member.id) in bag.content:
            prioritet += 1
          ...
          if str(member.id) in att.content:
            prioritet += 1
          ...
          elif member.id == 420506181627412501:
