        ...
        if i.id in idd:
          for j in i.text_channels:
            if j.id != 764911620111204383:
              b = await j.history(limit=200, after=datetime.datetime.strptime(data1, '%d.%m.%Y'), before=datetime.datetime.strptime(data2, '%d.%m.%Y')+datetime.timedelta(hours=24)).flatten()
              for k in b:
