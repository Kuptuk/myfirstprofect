...
async def on_raw_reaction_add(payload):
  ...
  if payload.message_id == 749327767061135502:
    ...
    global mods; global key; global mod2; global mods_type
    if payload.channel_id == id_chn_jb and payload.emoji.id == 785191301321719828 and 677397817966198788 in [role.id for role in gg.get_member(payload.user_id).roles]:
        key = 0
        ...
        msg_c = await client.get_channel(id_chn_jb).fetch_message(payload.message_id)
        await msg_c.delete()
        ...
        if payload.emoji.id == 819647171430973480:
            ...
            reason_b = 'доказательства устарели.'
        mods2.update({payload.message_id:reason_b})
        msg_c = await client.get_channel(id_chn_jb).fetch_message(payload.message_id)
        await msg_c.clear_reactions()
